...
# ── Récupération features et threshold ────────────────────────────────
    # Priorité 1 : MLflow (params du run Production)
    if prod_run_id:
        try:
            prod_run     = client.get_run(prod_run_id)
            threshold    = prod_run.data.metrics.get("threshold", threshold)
            feature_cols_str = prod_run.data.params.get("feature_cols", None)
            if feature_cols_str:
                import ast
                feature_cols = ast.literal_eval(feature_cols_str)
                print(f"  Features chargees depuis MLflow : {len(feature_cols)}")
        except Exception as e:
            print(f"  [WARN] Impossible de charger features depuis MLflow : {e}")
    ...
    if feature_cols is None:
        if os.path.exists(THRESHOLD_FILE):
            ...
            print(f"  Features chargees localement : {len(feature_cols)}")
        else:
            print(f"  [WARN] Pas de features disponibles — utilisation de toutes les colonnes")
